...
import { getVagonsServerSideProps } from '../../lib/getServerSideProps';
import { fetchVagons } from '../../api/vagons';
...
const VagonsPage = ({ vagons }: Props) => {
  const { data, isLoading, error } = useQuery<Vagon[]>({
    queryKey: ['vagons'],
    queryFn: fetchVagons,
    initialData: vagons,
  });
  ...
  const filterFn = (data: Vagon) =>
    searchVagonNumber === '' || data.VagonNumber.includes(searchVagonNumber);

  const sortFn = (() => {
    switch (sortOption) {
      case 'option1':
        return (a: Vagon, b: Vagon) => Number(a.VagonNumber) - Number(b.VagonNumber);
      case 'option2':
        return (a: Vagon, b: Vagon) => Number(b.VagonNumber) - Number(a.VagonNumber);
      case 'option3':
        return (a: Vagon, b: Vagon) => a.DepartureStationName.localeCompare(b.DepartureStationName);
      case 'option4':
        return (a: Vagon, b: Vagon) => b.DepartureStationName.localeCompare(a.DepartureStationName);
      default:
        return null;
    }
  })();
  ...
  const { paginated, filtered, pageCount, currentPage, setCurrentPage, total } =
    useFilterSortPaginate(data, filterFn, sortFn, visibleCount);
